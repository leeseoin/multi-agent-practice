...
| 내가 다 해줄 때 | 네가 직접 할 때 |
|----------------|----------------|
| 코드 복붙 → 금방 잊어버림 | 직접 타이핑 → 손가락이 기억함 |
| "아 되네" 하고 넘어감 | "왜 이렇게 하지?" 고민하게 됨 |
| 에러 경험 없음 | 에러 만나고 해결 → 진짜 실력 |

**진행 방식:**
1. 사용자가 질문 → "이거 어떻게 해?"
2. Claude는 힌트만 → "이런 방향으로 해봐", "이 함수 찾아봐"
3. 사용자가 코딩 → 직접 작성
4. 막히면 → 구체적으로 뭐가 안 되는지 보여주고 도움 요청
5. Claude가 피드백 → 코드 리뷰, 개선점 제안
